...
def rotx(angle):
    ...
    return (_n.array([[1, 0, 0], [0, _n.cos(angle), -_n.sin(angle)],
                      [0, _n.sin(angle), _n.cos(angle)]]))


def roty(angle):
    '''Generates a rotational matrix rotated angle around y axis'''
    return (_n.array([[_n.cos(angle), 0, _n.sin(angle)], [0, 1, 0],
...
def rotz(angle):
    ...
    return (_n.array([[_n.cos(angle), -_n.sin(angle), 0],
                      [_n.sin(angle), _n.cos(angle), 0], [0, 0, 1]]))
...
class Primitive(object):
    ...
    normals = None
    # if this primitive has to be translated and/or rotated put matrix here if
    # this primitive has a different color than the rest of the object then
    # defined it here
    trans_rot_matrix = None
    material_specular_color = None
    material_shininess = None
    material_ambient_color = None
    material_diffuse_color = None
    # we can also include a gl_list (this gets executed at the end of the
    # primitive
    gl_list = None
...
class Object_model(object):
    ...
    def set_axis(self, axis):
        # create matrix out of axis using a random vector and projecting it
        # into the perpendicular plane of the axis, the axis is the third
        # column or row of the rot matrix
    ...
    def add_primitive(self, primitive):
        ...
        # parses the primitive values and generates the gl commands using vbo
        # method (copying this information to gl memory) and generates a gl_
        # list that then is stored in gl_primitives
        self.counter += 1
        self.gl_primitives[self.counter] = primitive
        return (self.counter)

    def remove_primitive(self, primitive_id):
        ''' Give the primitive id returned by add_primitive,

        return True if an error occured
        '''
        # frees opengl memory for this vbo and gl_list data. Then erases the
        # entry in self.gl_primitives
    ...
    def gen_gl_list(self):
        # This function should be run only when the object shape is generated
        # or updated generating objects gl lists
        ...
        # offset of object
        if self.center_offset_matrix is not None:
            _gl.glPushMatrix()
        ...
        for object_index in self.object_models:
            ...
            if object.visibility:
                _gl.glPushMatrix()
                if object.scale is not None:
                    _gl.glEnable(_gl.GL_NORMALIZE)
                _gl.glMultMatrixf(object.trans_rot_matrix.T)
                if object.scale is not None:
                    _gl.glScale(object.scale[0], object.scale[1],
                                object.scale[2])
                if object.material_specular_color is not None:
                    _gl.glMaterialfv(_gl.GL_FRONT, _gl.GL_SPECULAR,
                                     object.material_specular_color)
                if object.material_shininess is not None:
                    _gl.glMaterialf(_gl.GL_FRONT, _gl.GL_SHININESS,
                                    object.material_shininess)
                if object.material_ambient_color is not None:
                    _gl.glMaterialfv(_gl.GL_FRONT, _gl.GL_AMBIENT,
                                     object.material_ambient_color)
                if object.material_diffuse_color is not None:
                    _gl.glMaterialfv(_gl.GL_FRONT, _gl.GL_DIFFUSE,
                                     object.material_diffuse_color)
        ...
        for primitive in self.gl_primitives:
            if self.gl_primitives[primitive].trans_rot_matrix:
                ...
                _gl.glPushMatrix()
                _gl.glMultMatrixf(
                    self.gl_primitives[primitive].trans_rot_matrix.T)
            if self.gl_primitives[primitive].vertices is not None:
                _gl.glBegin(self.gl_primitives[primitive].type)
                if self.gl_primitives[primitive].material_specular_color:
                    _gl.glMaterialfv(
                        _gl.GL_FRONT, _gl.GL_SPECULAR,
                        self.gl_primitives[primitive].material_specular_color)
                if self.gl_primitives[primitive].material_shininess:
                    _gl.glMaterialf(
                        _gl.GL_FRONT, _gl.GL_SHININESS,
                        self.gl_primitives[primitive].material_shininess)
                if self.gl_primitives[primitive].material_ambient_color:
                    _gl.glMaterialfv(
                        _gl.GL_FRONT, _gl.GL_AMBIENT,
                        self.gl_primitives[primitive].material_ambient_color)
                if self.gl_primitives[primitive].material_diffuse_color:
                    _gl.glMaterialfv(
                        _gl.GL_FRONT, _gl.GL_DIFFUSE,
                        self.gl_primitives[primitive].material_diffuse_color)
                if self.gl_primitives[primitive].normals is None:
                    for i in range(
                            self.gl_primitives[primitive].vertices.shape[0]):
                        _gl.glVertex3fv(
                            self.gl_primitives[primitive].vertices[i])
                    ...
                    for i in range(
                        ...
                        _gl.glVertex3fv(
                            self.gl_primitives[primitive].vertices[i])
                elif self.gl_primitives[
                        primitive].vertices.shape == self.gl_primitives[
                            primitive].normals.shape:
                    for i in range(
                            self.gl_primitives[primitive].vertices.shape[0]):
                        _gl.glNormal3fv(
                            self.gl_primitives[primitive].normals[i])
                        _gl.glVertex3fv(
                            self.gl_primitives[primitive].vertices[i])
                else:
...
class Light(object):
    LIGHTS = [
        _gl.GL_LIGHT0, _gl.GL_LIGHT1, _gl.GL_LIGHT2, _gl.GL_LIGHT3,
        _gl.GL_LIGHT4, _gl.GL_LIGHT5, _gl.GL_LIGHT6, _gl.GL_LIGHT7
    ]
...
class Scene(object):
    ...
    def set_ambient_light_color(self, ambient_light_color):
        ...
        _gl.glLightModelfv(_gl.GL_LIGHT_MODEL_AMBIENT,
    ...
    def add_light(self, light):
        # TODO check that light number is not higher that the maximum allowed
        # by open_gl.
    ...
    def draw_scene(self):
        # inspect all self.objects, and execute only those that have
        # visibility=True, inspect all lights and execute those ones
        # activated. Execute the ambient light
        # camera
        # lights
...
class Display(object):
    def __init__(self, camera, scene, res=(320, 240), image_dump_dir=""):
        '''Creates a display of resolution res'''
        self.camera = camera
        self.scene = scene
        self.size = res
        ...
        self.pg_surface = _pygame.display.set_mode(
            self.size, _pygame.OPENGL | _pygame.DOUBLEBUF)
        ...
        # vertex arrays
        self.cube_vertices = _n.array(
            [[-1., -1., -1], [-1, 1, -1], [1, 1, -1]], "f")
        self.cube_back_indices = _n.array([0, 1, 2])

        # buffer objets
        from OpenGL.arrays import vbo
        self.cube_vertices_vbo = vbo.VBO(
            self.cube_vertices, usage="GL_STATIC_DRAW")
        self.cube_vertices_vbo.bind()
        _gl.glBufferData(_gl.GL_ARRAY_BUFFER, self.cube_vertices_vbo.data,
                         _gl.GL_STATIC_DRAW)
        # _gl.glBufferSubData(_gl.GL_ARRAY_BUFFER,0,None,cub_vertices)
        self.cube_back_indices_vbo = vbo.VBO(
            self.cube_back_indices,
            usage="GL_STATIC_DRAW",
            target="GL_ELEMENT_ARRAY_BUFFER")
        self.cube_back_indices_vbo.bind()
        _gl.glBufferData(_gl.GL_ELEMENT_ARRAY_BUFFER,
                         self.cube_back_indices_vbo.data, _gl.GL_STATIC_DRAW)
    ...
    def update(self):
        ...
        if self.image_dump_dir != "":
            filename = self.image_dump_dir + "capture-" + str(
                self.init_time) + "-%07d.png" % (self.img_cnt)
...
class Camera_matrix(object):
    ...
    # We control with the mouse the azimuth, the inclination and the radius
    # (scrolling). With ctrl we control the center of the sphere X, Y and Z
    # with respect to the camera coordinates. And with Alt and scroll we
    # control the rotation of the camera around its normal.
    def __init__(self):
    ...
    def get_frame(self):
        ...
        self.center_rot_frame = _n.dot(
            _n.dot(rot_m, self.delta_pos_m), self.center_rot_frame)
    ...
    def update(self, event):
        if event.type == _pygame.MOUSEMOTION:
            if event.buttons == (1, 0, 0):
                self.camera_matrix.change_sphere(self.rot_inc * event.rel[0],
                                                 self.rot_inc * event.rel[1],
                                                 0.)
            if event.buttons == (0, 0, 1):
                self.camera_matrix.change_center(
                    _n.array([
                        self.xyz_inc * event.rel[0],
                        -self.xyz_inc * event.rel[1], 0.
                    ]))
            return ()
