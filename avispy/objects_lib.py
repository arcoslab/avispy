...
def gen_circle(radius, angle_step, z_pos):
    ...
    for angle in _n.arange(0., 360 * _n.pi / 180.0, angle_step):
        vertices.append(
            _n.array([radius * _n.sin(angle), radius * _n.cos(angle), z_pos]))
    return (vertices)
...
class Disk(_engine.Object_model):
    def __init__(self, radius=1.0, angle_step=10.0 * _n.pi / 180.0):
        ...
        primitive.vertices = _n.array(
            [_n.array([0., 0., 0.])] + vertices + [vertices[0]])
...
class Cone(_engine.Object_model):
    def __init__(self, radius=1.0, height=3.0,
                 angle_step=10.0 * _n.pi / 180.0):
        self.radius = radius
        self.height = height
        # cone side surface
        ...
        self.side_surface_prim.vertices = _n.array(
            vertices_temp + [vertices[0]])
        normals_temp = []
        # for each vertex in the base circle calculate the normal
        for vertex in vertices:
            normal_temp = _n.cross(
                _n.cross(tip - vertex, vertex), tip - vertex)
            normal_temp /= _ln.norm(normal_temp)
            normals_temp.append(normal_temp)
        # calculate the normal for each "tip" as the average of two
        # contiguous vertices
        normals_tip = []
        ...
        self.base_cap_prim.vertices = _n.array(
            [_n.array([0., 0., 0.])] + vertices + [vertices[0]])
...
class Triangle_3D(_engine.Object_model):
    def __init__(self, side=1.0, depth=1.0):
        ...
        self.side0_prim.vertices = _n.array([[
            -side / 2.0, -(side / 2.0) / _n.tan(60 * _n.pi / 180.0),
            -depth / 2.0
        ], [
            side / 2.0, -(side / 2.0) / _n.tan(60 * _n.pi / 180.0),
            -depth / 2.0
        ...
        self.side1_prim.normals = _n.array(
            [_n.sin(30 * _n.pi / 180.0),
             _n.cos(30 * _n.pi / 180.0), 0.0])
        ...
        self.side2_prim.normals = _n.array(
            [-_n.sin(30 * _n.pi / 180.0),
             _n.cos(30 * _n.pi / 180.0), 0.0])
...
class Cylinder(_engine.Object_model):
    def __init__(self, radius=1.0, height=1.0,
        ...
        vertices_temp = []
        normals_temp = []
        vertices_base, vertices_top = gen_cylinder(
            radius, height, angle_step=angle_step)
        ...
        self.base_cap_prim.type = _gl.GL_TRIANGLE_FAN
        self.base_cap_prim.vertices = _n.array(
            [_n.array([0., 0., 0.])] + vertices_base + [vertices_base[0]])
        self.base_cap_prim.normals = _n.array([0., 0., -1.])
        self.top_cap_prim = _engine.Primitive()
        self.top_cap_prim.type = _gl.GL_TRIANGLE_FAN
        self.top_cap_prim.vertices = _n.array(
            [_n.array([0., 0., height])] + vertices_top + [vertices_top[0]])
...
class Arrow(_engine.Object_model):
    def __init__(self):
        ...
        self.shaft_length = self.total_length / (
            1.0 + (1.0 / self.len_shaft_cone_ratio))
        self.cone_length = self.total_length - self.shaft_length
        _engine.Object_model.__init__(self)
        cylinder = Cylinder(
            radius=self.shaft_length / self.shaft_length_width_ratio,
            height=self.shaft_length)
        cone = Cone(
            radius=self.cone_length / self.cone_length_width_ratio,
...
class Bar(_engine.Object_model):
    def __init__(self, side1=1.0, side2=1.0, height=1.0):
        ...
        base_square_prim.vertices = _n.array([[
            -side1 / 2.0, -side2 / 2.0, 0.0
        ], [-side1 / 2.0, side2 / 2.0, 0.0], [side1 / 2.0, -side2 / 2.0, 0.0],
                                              [side1 / 2.0, side2 / 2.0, 0.0]])
        ...
        top_square_prim.vertices = _n.array(
            [[-side1 / 2.0, -side2 / 2.0,
              height], [-side1 / 2.0, side2 / 2.0,
                        height], [side1 / 2.0, -side2 / 2.0, height],
        ...
        side1a_square_prim.vertices = _n.array(
            [[-side1 / 2.0, -side2 / 2.0,
              0.0], [-side1 / 2.0, -side2 / 2.0,
                     height], [side1 / 2.0, -side2 / 2.0, 0.0],
             [side1 / 2.0, -side2 / 2.0, height]])
        ...
        side1b_square_prim.vertices = _n.array(
            [[-side1 / 2.0, side2 / 2.0,
              0.0], [-side1 / 2.0, side2 / 2.0,
                     height], [side1 / 2.0, side2 / 2.0, 0.0],
             [side1 / 2.0, side2 / 2.0, height]])
        ...
        side2a_square_prim.vertices = _n.array(
            [[-side1 / 2.0, -side2 / 2.0,
              0.0], [-side1 / 2.0, -side2 / 2.0,
                     height], [-side1 / 2.0, side2 / 2.0, 0.0],
             [-side1 / 2.0, side2 / 2.0, height]])
        ...
        side2b_square_prim.vertices = _n.array(
            [[side1 / 2.0, -side2 / 2.0,
              0.0], [side1 / 2.0, -side2 / 2.0,
                     height], [side1 / 2.0, side2 / 2.0, 0.0],
             [side1 / 2.0, side2 / 2.0, height]])
...
class Curve(_engine.Object_model):
    def __init__(self):
        ...
        self.y_min = 0.0
        self.center_offset_matrix = _n.identity(4)
        self.zero_mark_id = self.add_object_model(
            Line(_n.array([-1.0, 0.0, 0.0]), _n.array([1., 0., 0.])))
    ...
    def set_x_interval(self, interval):
        ...
        self.object_models[self.zero_mark_id].scale = [
            self.x_interval / (9 * 4), 1., 1.
        ]
        ...
        if self.previous_point is None:
            ...
            if len(self.segments) > 0:
                ...
                while point[0] - first_point[0] > self.x_interval:
                    ...
                    first_point = self.object_models[self.segments[0]].point1
                self.center_offset_matrix[:3][:, 3][
                    0] = -self.object_models[self.segments[0]].point1[0]
                self.object_models[
                    self.zero_mark_id].trans_rot_matrix[:3][:, 3][
                        0] = self.object_models[self.segments[0]].point1[0]
            self.previous_point = point
...
class Grid(_engine.Object_model):
    def __init__(self):
        ...
        horizontal_segments = [
            Line(
                _n.array([0., i, 0.]),
                _n.array([self.length - self.length / self.length_div, i, 0.]))
            for i in _n.arange(0.0, self.height, self.height / self.height_div)
        ]
        map(self.add_object_model, horizontal_segments)
        vertical_segments = [
            Line(
                _n.array([i, 0., 0.]),
                _n.array([i, self.height - self.height / self.height_div, 0.]))
            for i in _n.arange(0.0, self.length, self.length / self.length_div)
        ]
...
class Plot(_engine.Object_model):
    ...
    def set_y_div(self, y_div):
        ...
        for curve in self.curves:
            curve.scale = [
                self.grid.length / (self.x_div * self.grid.length_div),
                self.grid.height / (self.y_div * self.grid.height_div), 1.
            ]
    ...
    def add_curve(self, curve):
        ...
        curve.trans_rot_matrix[:3][:, 3][1] = ((
            self.grid.height - self.grid.height / self.grid.height_div) / 2)
...
class Segment(_engine.Object_model):
    def __init__(self, kdl_segment):
        ...
        self.link_trans = _n.array(
            [self.kdl_link_frame.p[i] for i in range(3)])
        self.link_trans_rot = _n.identity(4)
        self.link_trans_rot[:3, :3] = _n.array(
            [[self.kdl_link_frame.M[i, j] for j in range(3)]
             for i in range(3)])
        ...
        if self.joint_type != "None":
            ...
            if self.joint_type == "TransY" or self.joint_type == "TransZ" or \
               self.joint_type == "TransX":
    ...
    def set_angle(self, angle):
        ...
        if self.joint_type == "TransY" or self.joint_type == "TransZ" or \
           self.joint_type == "TransX":
            ...
            self.cylinder.scale = [
                self.bar.scale[0], self.bar.scale[0],
                abs(angle)
            ]
...
class Articulated():
    ...
    def set_angles(self, angles):
        ...
        if len(self.angles) != len(self.segments):
            print("Error, wrong number of angles, got", len(
                self.angles), " should be:", len(self.segments))
    ...
    def update_transformations(self):
        ...
        for segment, angle in zip(self.segments, self.angles):
            ...
            next_link_trans_rot = _n.dot(segment.trans_rot_matrix,
                                         segment.link_trans_rot)
...
class Framed_face(_engine.Object_model):
    ...
    def generate_frames(self):
        ...
        for i in range(x_sample_cant + 1):
            y = self.y_start
            theta = 0
            for j in range(y_sample_cant + 1):
                theta = 0
                for k in range(theta_sample_cant + 1):
...
class Framed_sphere(_engine.Object_model):
    ...
    def generate_frames(self):
        ...
        self.list_of_poses = []
        for i in range(phi_sample_cant + 1):
            theta = 0
            psi = 0
            for j in range(theta_sample_cant + 1):
                psi = 0
                for k in range(psi_sample_cant):
                    # this will iterate over the angles, now generate the
                    # poses. generate orientation part
                    rot_matrix = _n.dot(
                        _n.dot(
                            _engine.rotx(phi - _n.pi / 2),
                            _engine.roty(theta - _n.pi / 2)),
                        _engine.rotz(psi))
...
class Framed_box(_engine.Object_model):
    def __init__(self,
        ...
        # visualization
        xy_top_face = Framed_face(self.x_length, self.y_length, self.step,
                                  self.angle_step)
        xy_bottom_face = Framed_face(self.x_length, self.y_length, self.step,
                                     self.angle_step)

        xz_right_face = Framed_face(self.x_length, self.z_length, self.step,
                                    self.angle_step)
        xz_left_face = Framed_face(self.x_length, self.z_length, self.step,
                                   self.angle_step)

        yz_front_face = Framed_face(self.z_length, self.y_length, self.step,
                                    self.angle_step)
        yz_back_face = Framed_face(self.z_length, self.y_length, self.step,
                                   self.angle_step)
...
class Convex_hull(_engine.Object_model):
    def __init__(self, list_of_points):
        ...
        for face in self.faces:
            normal = _n.cross(-face[0, :3] + face[1, :3],
                              -face[0, :3] + face[2:3])
            prim = _engine.Primitive()
